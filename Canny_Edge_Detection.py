...
for i in sigma:
    ...
    plt.savefig(f'Results/{name}/{name}_smoothing_x-axis_{i}.jpg')

    D_x = convolve_x(S_x, gaussian_derivative_kernel(kernel_size, i))
    plt.imshow(D_x, cmap='gray')
    plt.savefig(f'Results/{name}/{name}_derivative_x-axis_{i}.jpg')

    S_y = convolve_y(I, gaussian_kernel(kernel_size, i))
    plt.imshow(S_y, cmap='gray')
    plt.savefig(f'Results/{name}/{name}_smoothing_y-axis_{i}.jpg')

    D_y = convolve_y(S_y, gaussian_derivative_kernel(kernel_size, i))
    plt.imshow(D_y, cmap='gray')
    plt.savefig(f'Results/{name}/{name}_derivative_y-axis_{i}.jpg')

    mag, dir = compute_gradient_magnitude_and_direction(D_x, D_y)
    plt.imshow(mag, cmap='gray')
    plt.savefig(f'Results/{name}/{name}_gradient_magnitude_{i}.jpg')
    plt.imshow(dir)
    plt.savefig(f'Results/{name}/{name}_gradient_direction_{i}.jpg')

    suppressed = non_maximum_suppression(mag, dir)
    plt.imshow(suppressed, cmap='gray')
    plt.savefig(f'Results/{name}/{name}_Non_Maximum_Suppression_{i}.jpg')
    ...
    plt.savefig(f'Results/{name}/{name}_Canny_Edge_Detection_{i}.jpg')